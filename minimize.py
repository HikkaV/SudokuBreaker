...
class Train:
    def __init__(self, path, seed=5, custom=True, test_portion=0.15):
        self.train_x, self.train_y, self.test_x, self.test_y = load_and_process(path, seed, test_portion)
        self.custom = custom
    ...
    def objective(self, space):
        ...
        if self.custom:
            sudoku_model.fit_custom(self.train_x, self.train_y, self.test_x, self.test_y,
                                    batch=batch, epochs=epochs, learning_rate=learning_rate, id_mlflow=id_mlflow)
        else:
            sudoku_model.fit_inbuilt(self.train_x, self.train_y, self.test_x, self.test_y,
